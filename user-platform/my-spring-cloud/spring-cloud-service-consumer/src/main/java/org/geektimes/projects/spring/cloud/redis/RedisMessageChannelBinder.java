...
public class RedisMessageChannelBinder extends AbstractBinder<MessageChannel, ConsumerProperties, ProducerProperties> {
    private final Logger logger = LoggerFactory.getLogger(RedisMessageChannelBinder.class);
    private static final String ERROR_HEADER = "errorKey";
    FileSystemPropertySourceLocator
                    ...
                    try {
                        return retryTemplate.execute(new RetryCallback<Boolean, Exception>() {
                            ...
                            public Boolean recover(RetryContext context) throws Exception {
                                RedisMessageChannelBinder.this.logger.error("Failed to deliver message; retries exhausted; message sent to queue 'ERRORS:"
                        ...
                        RedisMessageChannelBinder.this.logger.error("Failed to deliver message", e);
                        return false;
                    }
                }
    ...
    private RetryTemplate buildRetryTemplateIfRetryEnabled(ConsumerProperties properties) {
    ...
    private String buildPartitionRoutingExpression(String name) {
        return name;
    }
    ...
    private class SendingHandler extends AbstractMessageHandler {
        ...
        private SendingHandler(String bindingName, ProducerProperties producerProperties) {
            ...
            this.partitionHandler = new PartitionHandler(evaluationContext, producerProperties, beanFactory);
        ...
        private MessageValues serializePayloadIfNecessary(Message<?> message) {
